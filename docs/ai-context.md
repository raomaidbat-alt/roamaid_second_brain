...
## Что работает
*   **Основные компоненты:** Telegram бот (`/root/bot.py`), агент аудита Instagram (`/root/audit_agent.py`), анализ YouTube/Instagram роликов, генерация тредов (ПАРАДОКС/ЦИФРЫ/БОЛЬ), аудит Instagram/сайтов, обучение из YouTube (`/learn`).
*   **Выполненные задачи:** Анализ вирусных тредов (`/root/skills/analyze_threads/`) и публикация в Threads (`/root/skills/post_to_threads/`). Оба требуют `THREADS_USERNAME` и `THREADS_PASSWORD` в окружении.
...
1.  **Генерация каруселей** (`/root/skills/carousel_generator/`): Gemini генерирует текст слайдов, Playwright рендерит HTML→PNG (требуется Chromium). Поддержка форматов Threads (4:5), Instagram (1:1), Stories (9:16). Предусматривается предпросмотр и одобрение хозяином в Telegram.
