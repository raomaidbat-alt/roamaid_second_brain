### Статус: 30.05.2026 10:00
### Статус: 2024-05-16 10:30
```markdown
...
*   **Рабочие скиллы:** Анализ роликов (`/root/skills/analyze_reel/`), генерация тредов (`/root/skills/generate_thread/`), полный аудит Instagram (`/root/skills/audit_account/`), аудит сайтов (`/root/skills/audit_website/`), обучение из YouTube (`/root/skills/learn_from_video/`), анализ вирусных тредов (✅ `/root/skills/analyze_threads/`), публикация в Threads (✅ `/root/skills/post_to_threads/`).
...
1.  **Генерация каруселей** (`/root/skills/carousel_generator/`): Gemini генерирует текст слайдов, Playwright рендерит HTML → PNG (требует установки Chromium). Telegram показывает превью для одобрения. Форматы: Threads (4:5), Instagram (1:1), Stories (9:16).
2.  **Публикация в Threads с одобрением хозяина:** Использует `threads_api` для постинга, но только после одобрения хозяина через Telegram.
