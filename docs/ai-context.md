...
## Что работает
*   **Ядро:** AI-агент, управляемый через Telegram бот (`/root/bot.py`). Хозяин общается только через Telegram.
*   **Агенты/Скиллы:** `/root/audit_agent.py` (аудит Instagram). Работают скиллы для анализа YouTube/Instagram роликов, генерации тредов (ПАРАДОКС/ЦИФРЫ/БОЛЬ), аудита Instagram аккаунтов и сайтов, обучения из YouTube видео (`/learn`), анализа вирусных тредов (`/analyze_threads`) и публикации в Threads (`/post_to_threads`).
...
*   **Память:** Трехслойная архитектура: `/root/brain/memory/` (факты), `/root/brain/daily/` (ежедневные заметки), `/root/roamaid_second_brain/CLAUDE.md` (системный контекст/правила).
