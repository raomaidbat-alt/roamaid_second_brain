...
## Что работает
*   **Роль:** Автономный агент для роста в соцсетях, управление через Telegram.
...
1.  **Генерация каруселей** (`/root/skills/carousel_generator/`): Gemini генерирует текст, Playwright рендерит HTML → PNG (требует установки Chromium). Реализовать предпросмотр в Telegram и публикацию в Threads после одобрения хозяином. Поддержка форматов: Threads (4:5), Instagram (1:1), Stories (9:16).
