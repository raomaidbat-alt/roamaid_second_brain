...
1.  **Генерация каруселей** (`/root/skills/carousel_generator/`): Gemini генерирует текст, Playwright рендерит HTML → PNG (требует Chromium). Telegram показывает превью для одобрения хозяином. Поддержка форматов: Threads (4:5), Instagram (1:1), Stories (9:16).
