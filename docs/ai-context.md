### Статус: 24.05.2026 23:00
```markdown
# Roamaid Second Brain — Сжатый системный контекст

### Статус: 26.05.2024 10:30
...
*   **Основные компоненты:** `/root/bot.py` (Telegram бот), `/root/audit_agent.py` (аудит Instagram).
...
1.  **Генерация каруселей** (`/root/skills/carousel_generator/`): Gemini генерирует текст слайдов, Playwright рендерит HTML в PNG (форматы: Threads 4:5, Instagram 1:1, Stories 9:16). Требуется установка Chromium, предусмотрены превью и одобрение хозяином в Telegram.
