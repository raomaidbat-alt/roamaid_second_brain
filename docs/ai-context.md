...
*   **Скиллы:** Анализ роликов (`analyze_reel`), генерация тредов (`generate_thread`), полный аудит Instagram (`audit_account`), аудит сайтов (`audit_website`), обучение из YouTube (`learn_from_video`), анализ вирусных тредов (`analyze_threads`), публикация в Threads (`post_to_threads`).
...
1.  **Генерация каруселей** (`/root/skills/carousel_generator/`): Gemini генерирует текст, Playwright рендерит HTML в PNG (требует Chromium). Поддержка форматов Threads (4:5), Instagram (1:1), Stories (9:16). Публикация после одобрения хозяина в Telegram.
