...
from skills.funnel_analyzer.skill import analyze_funnels as funnel_analyzer_skill
try:
...
async def handle_analyze_funnels(update, context):
    text = update.message.text.split()
    usernames = [w for w in text[1:] if not w.startswith('category:')]
    if not usernames:
        await update.message.reply_text("Укажи ники через пробел")
        return
    await update.message.reply_text(f"⏳ Анализирую {len(usernames)}...")
    try:
        result = await funnel_analyzer_skill(usernames, None)
